...
#include "rqt_tutorial_2_rtsp_stream/tutorial_2.hpp"
...
namespace rqt_tutorial_2_rtsp_stream
{
...
void Dashboard::initPlugin(qt_gui_cpp::PluginContext& context)
{
  ...
  node_ = rclcpp::Node::make_shared("rqt_tutorial_2_rtsp_stream_rclcpp_node");
  ...
  player->setVideoOutput(ui_.rtsp);

  ui_.rtsp->show();

  player->setMedia(QUrl(QString("gst-pipeline: rtspsrc location=rtsp://%1:%2/%3 latency=0\
            ! decodebin ! videobalance contrast=1.2 brightness=-0.08 ! xvimagesink name= qtvideosink sync=false")
                            .arg(camIP)
                            .arg(camPort)
                            .arg(camPath)));
  player->play();
...
PLUGINLIB_EXPORT_CLASS(rqt_tutorial_2_rtsp_stream::Dashboard, rqt_gui_cpp::Plugin)
